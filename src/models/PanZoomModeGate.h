...
// MainWindow::togglePanZoomModeForPan). Only a Flex radio's command plane ever
...
// wants to explain itself names the outermost reason.
...
// `usesFlexCommandPlane` is RadioModel::usesFlexCommandPlane() -- a direct
// family() == "flex" test, not merely "a connection object exists". SimBackend
// owns a RadioConnection too and does not understand band_zoom=.
...
[[nodiscard]] constexpr PanZoomModeRefusal panZoomModeRefusal(
    bool connected, bool usesFlexCommandPlane, bool panKnown) noexcept
{
    if (!connected) {
        return PanZoomModeRefusal::NotConnected;
    }
    if (!usesFlexCommandPlane) {
        return PanZoomModeRefusal::NoFlexCommandPlane;
    }
    if (!panKnown) {
...
[[nodiscard]] constexpr bool panZoomModeWritable(
    bool connected, bool usesFlexCommandPlane, bool panKnown) noexcept
{
    return panZoomModeRefusal(connected, usesFlexCommandPlane, panKnown)
...
[[nodiscard]] constexpr bool bandSegmentZoomAvailable(
    bool connected, bool usesFlexCommandPlane) noexcept
{
    return panZoomModeWritable(connected, usesFlexCommandPlane, /*panKnown=*/true);
}
