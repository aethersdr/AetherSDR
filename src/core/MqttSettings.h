...
// Payload contract lives in core/MqttRadioState.h. Two things a subscriber must
// know and cannot infer from a sample message: fields are keyed by PRESENCE (an
// absent `drive` means "not reported", never 0), and the topic is not published
// mid-CWX — once tx:true has gone out for a CWX send, nothing republishes until
// the send ends, so the last message can predate the current transmission.
