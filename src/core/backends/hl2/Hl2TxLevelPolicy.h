...
//
// 50 is unity, and stays unity now that the level is restored per radio at
// connect (RadioStateMemory, ClientSettingsDomain::TxSetpoints): 50 is what a
// radio with nothing stored is restored to, so that session must leave the
// modulator exactly at its own 1.0 default. The persistence changed which
// sessions arrive here at 50; it did not retire the pin, and moving unity would
// still re-level every install that never asked for it. +/-20 dB across the
// travel, linear in dB.
//
