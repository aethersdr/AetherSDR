...
//
// 50 IS UNITY, and that is load-bearing rather than cosmetic. RadioModel now
// restores this radio's own stored mic level at connect (RadioStateMemory,
// ClientSettingsDomain::TxSetpoints), so the slider can arrive here carrying the
// operator's last position rather than a fresh 50 — but 50 is what a radio with
// nothing stored is restored to, and that session must still leave the
// modulator exactly where its own default (m_micGain = 1.0) puts it. A mapping
// with unity anywhere else would silently change the transmit level of every
// existing HL2 install, both the first time this code shipped and the first
// launch after the level began persisting.
//
