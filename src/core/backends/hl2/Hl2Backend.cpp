                               ...
                               TxAudioSource source)
{
    ...
    //
    // WHAT PROVIDES THE EXCLUSION IS NOT ONE GATE, and tciAudioFresh() is not
    // the one that covers engine audio. AudioEngine steps local mic capture
    // aside for a TCI/DAX source through that gate, but it only arms it for
    // markExternalSource — feedDaxTxAudioInternal starts m_tciAudioTimer under
    // that flag alone, and both engine-generated feeds pass it false. Each of
    // those suppresses the mic path itself instead: setDaxTxMode(true) in
    // AudioEngine::startWsprPump() and in Ax25HfPacketDecodeDialog, and the
    // radeMode early return in onTxAudioReady(). A fourth engine-generated
    // feed that forgets to do the same reintroduces this silently. Whoever
    // touches the mic-capture gate — or adds such a feed — owns re-checking.
