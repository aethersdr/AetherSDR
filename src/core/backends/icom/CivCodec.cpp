...
std::vector<std::uint8_t> cmdSetRepeaterAccess(std::uint8_t to, std::uint8_t mode)
{
    ...
    return buildFrameSub(to, cmd::kFunction, repeaterAccess::kFunction, body);
}
...
std::vector<std::uint8_t> cmdSetRepeaterOffset(std::uint8_t to, std::uint64_t hz)
{
    ...
    const std::array<std::uint8_t, 3> body{
        static_cast<std::uint8_t>((((units / 10ULL) % 10ULL) << 4) | (units % 10ULL)),
...
std::vector<std::uint8_t> cmdReadXfc(std::uint8_t to)
{
    return buildFrameSub(to, cmd::kControl, control::kXfc);
}

std::vector<std::uint8_t> cmdSetXfc(std::uint8_t to, bool enabled)
{
    const std::array<std::uint8_t, 1> body{static_cast<std::uint8_t>(enabled ? 1 : 0)};
