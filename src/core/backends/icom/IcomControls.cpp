...
constexpr std::array kSpecs = {
    ...
    ControlSpec{"repeater.access", 0x16, 0x5D, true, "FM repeater access",
                Plane::Slice, Encoding::Enum, Wiring::Both,
                0, 9, "mode", 0, 9,
                "setFmRepeaterAccess", "sliceFmToneMode", true,
                "IC-9700 profile only. 00 off, 01 TONE, 02 TSQL, 03 DTCS, "
                "06 DTCS(T), 07 TONE(T)/DTCS(R), 08 DTCS(T)/TSQL(R), "
                "09 TONE(T)/TSQL(R)."},

    ControlSpec{"repeater.offset", 0x0D, 0, false, "Repeater offset",
                Plane::Slice, Encoding::BcdFreq, Wiring::Both,
                0, 999999, "100 Hz", 0, 99999900,
                "setFmRepeaterOffset", "sliceFmOffset", true,
                "IC-9700 profile only. Read is 0C; direction is the separate 0F row."},
    ControlSpec{"repeater.duplex", 0x0F, 0, false, "Duplex direction",
                Plane::Slice, Encoding::Enum, Wiring::Both,
                0x00, 0x12, "mode", 0, 2,
                "setFmRepeaterOffset", "sliceFmDuplex", true,
                "IC-9700 profile only: write 10/read 00 simplex, 11 DUP-, 12 DUP+."},
    ControlSpec{"repeater.tone.tx", 0x1B, 0x00, true, "TX CTCSS tone",
                Plane::Slice, Encoding::BcdByte, Wiring::Both,
                0, 9999, "0.1 Hz", 670, 2541,
                "setFmRepeaterAccess", "sliceFmTxTone", true,
                "IC-9700 profile only; canonical EIA/TIA choices are boundary validated."},
    ControlSpec{"repeater.tone.rx", 0x1B, 0x01, true, "RX CTCSS tone",
                Plane::Slice, Encoding::BcdByte, Wiring::Both,
                0, 9999, "0.1 Hz", 670, 2541,
                "setFmRepeaterAccess", "sliceFmRxTone", true,
                "IC-9700 profile only; uses the same normalized presentation as TX."},
    ControlSpec{"repeater.dtcs", 0x1B, 0x02, true, "DTCS code and polarity",
                Plane::Slice, Encoding::BcdByte, Wiring::Both,
                0, 999, "code", 23, 754,
                "setFmRepeaterAccess", "sliceFmDtcs", true,
                "IC-9700 profile only. Payload byte 0 carries independent TX/RX polarity."},
    ...
    // ---- Control (0x1C) --------------------------------------------------
    ControlSpec{"repeater.reverse", 0x1C, 0x02, true, "XFC / REV",
                Plane::Slice, Encoding::OnOff, Wiring::Both,
                0, 1, "on/off", 0, 1,
                "setFmRepeaterReverse", "sliceFmReverse", true,
                "IC-9700 profile only. Temporary transmit-frequency monitor; not a duplex mode."},
    ControlSpec{"repeater.tx.frequency", 0x1C, 0x03, true,
                ...
                0, 0, "Hz", 0, 0,
                "", "sliceFrequencyPresentation", true,
                "IC-9700 profile only. Read-only authority for RX/TX presentation."},
