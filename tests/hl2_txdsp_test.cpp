...
// Every feed in this file goes through here, for two reasons that arrived
// separately and now both apply.
//
// PACED, at the cadence the live caller feeds blocks (see kFeedPaceUs). A TXA
// channel is asynchronous and a test that feeds a second of audio in a tight
// loop measures the queue rather than the modulator.
//
// AND CARRYING A TxCoordinator CONTEXT, which #5659 made a required argument of
// processAudioBlock: the dispatch guard drops a block whose context does not
// permit it, and a mismatched context resets the stage. Threading it through
// one helper rather than every call site is what stops a future call site
// forgetting it -- a forgotten context does not fail to compile, it silently
// feeds nothing.
