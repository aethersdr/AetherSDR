...
int main(int argc, char** argv)
{
    ...
    // meter slot and the status bar reads an overflow flag as telemetry.
    {
        ...
        // IC-9700 COMP is a continuous 0..100 control. Exercise the actual
        // scheduler and fake-radio readback so a UI-only range change cannot
        // conceal a broken CI-V encoder or missing confirmation read.
        std::optional<int> reportedCompLevel;
        QObject::connect(&c.backend, &IRadioBackend::transmitChanged, &c.backend,
                         [&reportedCompLevel](const TransmitDelta& delta) {
                             if (delta.speechProcLevel) {
                                 reportedCompLevel = *delta.speechProcLevel;
                             }
                         });
        const auto verifyContinuousComp = [&](int percent, int expectedRaw) {
            c.radio.clearCivLog();
            reportedCompLevel.reset();
            c.backend.setSpeechProcessor(true, percent);
            check(waitFor([&] { return reportedCompLevel == percent; }, 3000),
                  qPrintable(QStringLiteral("IC-9700 COMP %1 readback is authoritative")
                                 .arg(percent)));
            const auto& frames = c.radio.civCommands();
            const auto write = std::ranges::find_if(frames, [](const CivFrame& frame) {
                return frame.cmd == cmd::kLevel && frame.hasSub
                    && frame.sub == level::kCompLevel && !frame.data.empty();
            });
            check(write != frames.end() && decodeLevel(write->data) == expectedRaw,
                  qPrintable(QStringLiteral("IC-9700 COMP %1 writes raw %2")
                                 .arg(percent).arg(expectedRaw)));
            check(std::ranges::any_of(frames, [](const CivFrame& frame) {
                      return frame.cmd == cmd::kLevel && frame.hasSub
                          && frame.sub == level::kCompLevel && frame.data.empty();
                  }),
                  qPrintable(QStringLiteral("IC-9700 COMP %1 schedules confirmation read")
                                 .arg(percent)));
        };
        verifyContinuousComp(0, 0);
        verifyContinuousComp(50, 128);
        verifyContinuousComp(100, 255);

        c.backend.disconnectRadio();
    }

    // Sibling Icom profiles retain the established three-preset encoder.
    for (const auto [address, name] : std::array{
             std::pair<std::uint8_t, const char*>{0xA4, "IC-705"},
             std::pair<std::uint8_t, const char*>{0xB6, "IC-7300MK2"}}) {
        CivCase c(address, name);
        c.backend.connectRadio(c.request());
        check(waitFor([&] {
                  return c.backend.isConnected()
                      && c.backend.model().civAddress == address;
              }), "sibling Icom processor fixture connects");
        for (const auto [preset, expectedRaw] : std::array{
                 std::pair{0, 76}, std::pair{1, 153}, std::pair{2, 229}}) {
            c.radio.clearCivLog();
            c.backend.setSpeechProcessor(true, preset);
            check(waitFor([&] {
                      return std::ranges::any_of(
                          c.radio.civCommands(), [](const CivFrame& frame) {
                              return frame.cmd == cmd::kLevel && frame.hasSub
                                  && frame.sub == level::kCompLevel
                                  && !frame.data.empty();
                          });
                  }), "sibling Icom processor preset reaches the wire");
            const auto& frames = c.radio.civCommands();
            const auto write = std::ranges::find_if(frames, [](const CivFrame& frame) {
                return frame.cmd == cmd::kLevel && frame.hasSub
                    && frame.sub == level::kCompLevel && !frame.data.empty();
            ...
            check(write != frames.end() && decodeLevel(write->data) == expectedRaw,
                  qPrintable(QStringLiteral(
                      "IC-705/IC-7300MK2 preset %1 retains raw encoding %2")
                                 .arg(preset).arg(expectedRaw)));
            check(waitFor([&] {
                      return std::ranges::any_of(frames, [](const CivFrame& frame) {
                          return frame.cmd == cmd::kLevel && frame.hasSub
                              && frame.sub == level::kCompLevel && frame.data.empty();
                      });
                  }), "sibling Icom processor preset schedules confirmation read");
        }
        c.backend.disconnectRadio();
    }
