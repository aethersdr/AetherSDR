...
static void testIc9700DerivedForwardPowerAcrossBands()
{
    ...
    for (const QList<QVariant>& args : definitionSpy) {
        ...
        if (def.source == QStringLiteral("TX")
            && def.name == QStringLiteral("FWDPWR")) {
            wattsDefinition = def.unit == QStringLiteral("Watts");
        }
    }
    check(wattsDefinition,
    ...
    if (ic705) {
        IcomCivBackendTestAccess::selectModelAndFrequency(
            backend, *ic705, 14'100'000ULL);
        po.from = 0xA4;
        const int beforeIc705 = updateSpy.count();
        IcomCivBackendTestAccess::deliver(backend, po, 1);
        check(updateSpy.count() == beforeIc705 + 1
    ...
    check(ic7300mk2 != nullptr,
          "cross-radio power fixture resolves the IC-7300MK2");
    if (ic7300mk2) {
        IcomCivBackendTestAccess::selectModelAndFrequency(
            backend, *ic7300mk2, 14'100'000ULL);
        po.from = 0xB6;
        const int beforeIc7300 = updateSpy.count();
        IcomCivBackendTestAccess::deliver(backend, po, 1);
        check(updateSpy.count() == beforeIc7300 + 1
    ...
    if (ic705) {
        IcomCivBackendTestAccess::selectModelAndFrequency(
            backend, *ic705, 14'100'000ULL);
        po.from = 0xA4;
        const int beforeNativePower = updateSpy.count();
        IcomCivBackendTestAccess::deliver(backend, po, 1);
        check(updateSpy.count() == beforeNativePower + 1,
