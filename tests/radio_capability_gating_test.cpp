...
int main(int argc, char** argv)
{
    ...
    model.connectToRadio(hl2Info());
    {
        ...
        check(caps.hasAgcThreshold, "HL2 retains host AGC threshold");
        check(!caps.hasFullDuplex,
    ...
    // ---- Icom model capabilities and CI-V dial lock without a socket -------
    {
        ...
        if (ic7300Mk2) {
            ...
            IcomCivBackendTestAccess::prepareDialLockWrite(backend, *ic7300Mk2);
            ...
            check(!caps.hasCwTune, "MK2 does not advertise an unimplemented CW tune carrier");
            {
                ...
                IcomCivBackendTestAccess::prepareDialLockWrite(polled, *ic7300Mk2);
            ...
            for (const bool reverse : {false, true}) {
                ...
                IcomCivBackendTestAccess::prepareDialLockWrite(cwBackend, *ic7300Mk2);
        ...
        for (const char* name : {"IC-7300MK2", "IC-705"}) {
            ...
            for (std::size_t h = 0; h < profile->highEdgesHz.size(); ++h) {
                for (std::size_t l = 0; l < profile->lowEdgesHz.size(); ++l) {
                    ...
                    IcomCivBackendTestAccess::prepareDialLockWrite(writer, *model);
