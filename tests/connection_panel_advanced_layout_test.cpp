...
// than their own minimum heights. The source-path QComboBox rendered 14px tall
// (it asks for 25) and the word-wrapped hint above it was clipped to a sliced
// single line that overlapped the row below.
//
// ConnectionPanel::refitToContent() is what keeps the dialog tall enough. These
// tests assert the observable consequence rather than the mechanism: neither
// widget in the advanced section is ever allocated less height than it asks
// for. Both are load-bearing — stubbing refitToContent() to a no-op drops the
// combo to 10px (of 25) and the hint to 10px (of 30), reproducing the report.
//
// Deliberately not asserted here: the rows above the section. They keep their
// minimums even when the dialog cannot grow, because the squashing is absorbed
// by the section's word-wrapped labels first — so such a check passes in both
// states and would prove nothing.
...
void showOnManualPage(ConnectionPanel& panel)
{
    panel.show();
...
void expandAdvancedSection(ConnectionPanel& panel)
{
...
private slots:
    ...
    void sourcePathComboKeepsItsMinimumHeight()
    {
        ...
        showOnManualPage(panel);
        expandAdvancedSection(panel);
...
QTEST_MAIN(ConnectionPanelAdvancedLayoutTest)
