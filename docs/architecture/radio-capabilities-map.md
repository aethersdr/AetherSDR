...
| `hasJ16FilterBoard` | ❌ | ✅ | ❌ | `RadioSetupDialog` (Filter Board page), `Hl2Backend::applyBandFilter`, `AutomationServer::invokeExtension` (`filterboard.*` verbs) | Shows the Filter Board page and enables the manual per-band/per-direction J16 relay override; when `Hl2FilterBoardSettings::manualEnabled()` is true, `applyBandFilter()` consults the manual table instead of `ocFilterByteForHz()`. Unconditionally ✅ for the HL2 — the writes are inert with nothing listening on the I2C bus when no board is attached, same reasoning as the config-register writes. |
...
| `extensionNamespaces` | `["flex"]` | `["hl2"]` | — | No production reader or general `invokeExtension` pre-check yet | Flex: amp / tuner operate/bypass/autotune verbs. HL2: `freqcal.get` / `.set` / `.set_live`, behind the `freqcal` bridge verb and the Calibration page; `filterboard.get` / `.setManualEnabled` / `.setBand` / `.restoreDefaults`, behind `hasJ16FilterBoard` and the Filter Board page. Icom: `["icom"]`, with PC-audio, scope, control-map, scheduler and diagnostic verbs. RadioModel's Icom PC-audio wrappers and the Flex accessory routes still pre-check by family string; #5262 M1 converts those consumers. |
