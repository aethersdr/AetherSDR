...
| `driveIsReadback` | ✅ | ❌ | ❌ | `MainWindow::publishRadioStateMqtt` → `aethersdr/radio/state` `drive_confirmed` (#5518) | Whether `TransmitModel::rfPower()` is what the RADIO reports or what the OPERATOR asked for. Flex parses `transmit rfpower=` off status and Icom reads the CI-V RF-power level, so both are ✅. HL2 is ❌ by design: `setTxPower()` records intent before the transmit gate and `applyDrive()` pins the drive register at 0 while TX is blocked, so `rfPower()` can read 100 with no RF out. Sim/RTL/ANAN are ❌ — they populate no `TransmitDelta::rfPower`, so nothing publishes drive for them at all. Default is ✅ (the established shape for every backend that reports drive today) and every backend declares the choice explicitly |
