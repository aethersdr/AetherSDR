...
| `hasRadioDialLock` | ❌ | ❌ | ❌ | `RadioModel` → every `SliceModel` lock surface | Radio-authoritative global dial lock. Icom: ✅ only for the verified IC-9700 `16 50` path; front-panel/readback state fans out to RX Controls and every slice VFO |
