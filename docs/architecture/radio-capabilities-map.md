...
| `fmRepeaterPresentation` / `fmRepeaterAccessModes` / `hasFmRepeaterDuplex` / `hasFmRepeaterReverse` | Legacy / empty / ❌ / ❌ | Legacy / empty / ❌ / ❌ | Legacy / empty / ❌ / ❌ | `MainWindow::applyCapabilitiesToUi` → `RxApplet::setFmRepeaterCapabilities` and `VfoWidget::setFmRepeaterPresentation` | Selects the established legacy surface, a hidden unsupported surface, or model-evidenced extended controls without branching on `family`. Icom defaults to Hidden and publishes Extended plus individual functions only for the narrow IC-9700 profile. The VFO's legacy-only duplicate surface is visible only for Legacy; Extended state is presented by the authoritative RX Controls surface until the VFO implements that contract end-to-end |
