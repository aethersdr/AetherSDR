...
| `fmRepeaterPresentation` / `fmRepeaterAccessModes` / `hasFmRepeaterDuplex` / `hasFmRepeaterReverse` | Legacy / empty / ❌ / ❌ | Legacy / empty / ❌ / ❌ | Legacy / empty / ❌ / ❌ | `MainWindow::applyCapabilitiesToUi` → `RxApplet::setFmRepeaterCapabilities` | Selects the established legacy surface, a hidden unsupported surface, or model-evidenced extended controls without branching on `family`. Icom defaults to Hidden and publishes Extended plus individual functions only for the narrow IC-9700 profile |
