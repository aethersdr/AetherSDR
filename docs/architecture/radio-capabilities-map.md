...
| `hasTuner` | ✅ | ❌ | ❌ | `TransmitModel::setHasTuner` → `TxApplet` | Enables ATU / MEM when present; absent controls remain visible-disabled unless the presentation capability below says to hide them. Icom opts in only the evidenced IC-705 external-tuner and IC-7300MK2 tuner paths. |
| `hideUnavailableTunerControls` | ❌ | ❌ | ❌ | `TransmitModel::setHideUnavailableTunerControls` → `TxApplet` | Hides the ATU / MEM buttons and status indicators, while retaining their layout allocation, only when both this is true and `hasTuner` is false. Icom: true only for the exact IC-9700 profile; false for HL2/Sim preserves their established visible-disabled controls and reachable explanation. |
