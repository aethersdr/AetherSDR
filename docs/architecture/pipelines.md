...
- **SystemInfoCollector thread**: per-thread CPU and process-memory sampler behind Help → Runtime Monitor (#2554). On-demand: started when the dialog is shown, torn down when it is hidden. Every 1.5 s it takes one `ProcessMemorySnapshot::capture()` (emitted as `memorySampleReady` for the Memory tab, before the thread enumeration so a platform whose enumeration fails still gets a memory reading) and runs `SystemInfo::enumerateThreads()` off the GUI thread, emitting `sampleReady` / `thresholdExceeded` / `cpuSampleReady` (the process-level reading for the Overview tab: share of the whole machine, busiest thread, the threads with a non-zero share this tick) queued back to the dialog, so the sampler is not measured by the metric it gathers. The Overview's GUI tick-lag figure is NOT gathered here: it is read on the GUI thread from `UiTickLagMeter`, fed by MainWindow's 50 ms perf-heartbeat slot, when each CPU sample lands.
