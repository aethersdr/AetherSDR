...
| VFO B | `vfo:<trx>,1` targets a distinct TX slice when available. Resolution order is external TX slice, existing non-RX slice, then a new slice on the RX pan. | #1686, #1807, #2102, JTDX and WSJT-X Rig sequences. | `TciRoutingState::resolveVfoB()` and table-driven tests. |
...
| Split state | `split_enable` is explicit shared TCI state, not inferred from transient TX-slice topology. A true request is confirmed only after a distinct external, reused, or created TX route is selected. | Thetis exposes one global `VFOSplit`; #1686. | `TciRoutingState::setSplitRequested()` and ordered route continuations in `TciServer::handleSplitRequest()`. |
