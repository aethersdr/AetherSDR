...
| Client identity | A client's PTT resolves against the receiver it declared in `audio_start:<n>`; a client that declared none keeps the wire index. Replies always echo the trx the client sent. | #4547 — every WSJT-X instance addresses `trx:0`, so the wire cannot distinguish two instances. Thetis scopes RX-audio receiver sets per client while radio state stays global. | `TciServer::effectiveTrx()`; `pttBindsToTheDeclaredAudioReceiver`. |
| Keying resolution is strict | A receiver that does not resolve is declined with `trx:<n>,false;`, never silently addressed to the first slice. | #4547; the compatibility first-slice guess is fine for a read and transmits on an unaddressed slice under PTT. | `TciProtocol::resolveSliceForTrxStrict()` on the `handleTrxRequest` path. |
