...
| Changed tuning | A changed frequency is confirmed after the Flex command succeeds, and the success reply is also where model state adopts it. The confirmation carries the requested coordinate, never a read-back. | An accepted tune answers `R<seq>|0|` with an empty body, and the radio emits no slice status for a tune this client issued — the raw command never touches SliceModel — so the requested value is the only coordinate available and a read-back in the reply can only observe the pre-tune one. #4493 (~270 commands across every captured session, zero `RF_frequency` follow-ups). | `TciServer::handleTuneReply()` on `code == 0` calls `publishSliceFrequency()`, which writes via `SliceModel::applyChanges()` — never `setFrequency()`, which would re-issue the command. Covered by `tci_server_review_test`. |
| Rejected tuning | A rejected tune reports a coordinate from the reply body when it carries one, and otherwise the PRE-tune coordinate — the last value known to be true — rather than going silent. | Silence costs WSJT-X's 2 s `do_frequency()` timeout. No observed rejection supplies a coordinate: across every captured session every non-zero reply body is prose or empty (a locked slice answers `50000068|Unable to tune a locked slice -- unlock first`), so the body parse is unexercised and retained per #4493's triage. | `TciServer::handleTuneReply()` takes the pre-tune frequency as its fallback; `tci_server_review_test` covers the prose and empty bodies. |
