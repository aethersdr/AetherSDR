...
| VFO B | `vfo:<trx>,1` targets a distinct TX slice when available. Resolution order is an external radio-selected TX slice, a previously tracked route, then a new slice on the RX pan. An arbitrary non-TX slice is never treated as spare because it may be an operator's independent receiver. A TX slice that another connected client currently operates as its receiver is not adopted either: without a split request the frame is answered with the RX slice's own frequency and nothing is tuned. "Currently operates" means that client has a declared `audio_start:<n>` receiver that resolves strictly to the slice at that moment; before its `audio_start` arrives, after `audio_stop`, or while the trx map cannot resolve it, the slice is unclaimed and the adoption above applies (fail-open by design — a guess never refuses a legitimate tune). | #1686, #1807, #2102, #5193 (two WSJT-X instances, Split = Rig: the non-TX instance's channel-1 frame retuned the other instance's slice), JTDX and WSJT-X Rig sequences. | `TciRoutingState::resolveVfoB()`; `tci_trxmap_test` routing section; `TciServer::routingEndpoints()` ownership join pinned in `tci_server_review_test`. |
...
| Live TX beats the cache | A cached route never outvotes the live TX slice. When they disagree the live slice wins and the cache is refreshed, dropping TCI ownership so a later split teardown cannot `slice remove` a slice TCI never created. Exception: a live TX slice that another client currently operates as its receiver is never a PTT target, whatever the cache says — a route bound onto it while it was unclaimed is dropped (external bind) or kept but refused (TciCreated, so teardown can still remove it), and PTT keys the requested slice; a negotiated slice that is not the live TX slice is still promoted (#5193). | #4547 secondary: a route bound while slice 0 held TX survived the operator moving TX from the GUI, and a bare PTT then keyed slice 0's band and antenna. | `TciRoutingState::resolvePttSlice()`; `testStaleRouteFailsSafe`. |
...
| `(trx, channel 1)` | Resolved radio-global TX slice for that RX route; echo-only while that slice is another client's currently declared receiver and no split is requested (#5193); unclaimed slices route as before |
