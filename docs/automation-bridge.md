...
| `gps` | — | GPS status, tracked/visible counts, grid, radio-format coordinates, altitude, speed, course, UTC time, frequency error, configured NTP server address, and oscillator-reference state. This authenticated diagnostic response contains precise location data; the compact status bar and tooltip do not. |
