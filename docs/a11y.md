...
## Capability-gated controls: the reason must be announced

A control the radio cannot support is **dimmed with a reason**, never hidden
(#5262 M3a). A hidden control is not announced at all, so a screen-reader user
has no way to learn the radio simply does not support it — which is why the
previous "hide it" rule was reversed.

The reason has to travel on a channel accessibility clients actually read:

| Target | Channel | Note |
|---|---|---|
| `QWidget` | `setAccessibleDescription()` | plus `setToolTip()` for sighted users |
| `QAction` | `setStatusTip()` | `QAction` has no accessible description |

**A tooltip alone is not enough.** Qt exposes it as help text, which does
not replace the primary accessible description. This has regressed twice — #5266 shipped the full set on the Enforce
Private IP button and #5299 deleted it four days later; TX Band Settings and
Inhibit-during-TUNE shipped with `setEnabled` + `setToolTip` and no accessible
channel at all. `tools/check_a11y.py` now warns on that exact shape
(`a11y-disabled-reason-not-announced`).

It matters more than it looks: the two muted states are separated by only
1.39:1 in the light theme, so **colour does not reliably distinguish them** and
the description is the channel that actually carries the difference. See
`docs/style/theme-style-guide.md` §"Three-state controls" for the measured
contrast.

`ControlAvailabilityRegistry` sets both channels for you; prefer it to writing
the pair by hand.
