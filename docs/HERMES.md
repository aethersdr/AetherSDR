...
| Spectrum | WDSP analyzer (returns pixels) | Own `Hl2Spectrum` FFT + sample/average/peak detector | A3 §4 recommends exactly this for our architecture. Plan 3 added averaging (`setAveraging(frames, weighted)` — dB-domain EMA or boxcar) and Plan 5 the **peak/max-hold** detector (`setPeakHold(bool)` — infinite hold, or a sliding max over the FFT AVG window), both wired to the Display overlay and to `pan average` / `pan peakhold`. Span-following **bin count** (issue #1 story 8) is a named fast-follow — the parity lever is the detector, not FFT size (§12.4), so it does not gate the label; see ADR 0001. |
