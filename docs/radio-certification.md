...
| `TX:ALCGAIN` | dB | **host-side** | gain the ALC is applying | drive the input **above** `alcTargetPeak` → reading goes negative by the reduction applied; below the target it reads **0 dB** | **0 dB below the target, ±1 dB above it** — the stage is reduction-only with a unity ceiling on every path, so there is no window in which it lifts and a "sweep moves it 20 dB the other way" criterion would report a healthy meter as broken. `TxAudioSource::EngineGenerated` audio additionally bypasses the mic multiplier, so a mic-slider sweep moves this reading 0 dB for the WSPR, AX.25 and RADE sources by construction |
